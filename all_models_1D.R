...
lin.LS.ESS <- function(y,x,N1,M,nu,l,eta,nsim,burn.in,thin,tau.in,sig.in,xi.in,A,B,
  ...
  # return.plot : logical; if true a plot of estimate with 95% CI is returned; default is TRUE
  # tol: tolerance for numerical stability
  # A,B: matrix and vector of linear constraints, respectively (Axi+B>=0_m), where m nb of linear constr
  # prior : method for sampling prior (LS.KLE or Fast.LS); default 'Fast.LS'
  ...
  for (i in 1 : em) {
    ...
    if (condition == TRUE) {
      if (prior == 'Fast.LS') {
        ...
        nu.ess <- LS.KLE(u=my_knots,M=M,N1=N1,nu=nu,l=l,tausq=tau,tol=tol,sseedLS=i)
      xi_out <- ESS.linear(y=y,X=X,beta=xi_in,nu_ess=nu.ess,sigsq=sig,eta=eta,A=A,B=B,seeds=i)
    }
    set.seed(2*i)
    # sampling \sigma^2:
    Xxi <- as.vector(X %*% xi_out)
  ...
  return(list("time" = tm, "xi_sam" = xi_sam, "sig_sam" = sig_sam, "tau_sam" = tau_sam,
              "fhat_sam" = fhat_sam,"fmean" = fmean,
              "f_low" = f_low,"f_upp" = f_upp,"z_star" = z_star,
              "MAP" = MAP,"knots" = my_knots,"z_mean" = z_mean))
}
###################################################
...
lin.HMC <- function(y,x,N,nu,l,nsim,burn.in,thin,tau.in,sig.in,xi.in,A,B,
  ...
  # verbose : logical; if TRUE, prints current status; default is TRUE
  # return.plot : logical; if true a plot of estimate with 95% CI is returned; default is TRUE
  # A, B: matrix and vector of linear constraints, respectively
  # OUTPUT: Posterior samples on xi,tau,sig and fhat with posterior mean, 95% CI of fhat, mode of posterior distribution z_star
